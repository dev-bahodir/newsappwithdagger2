...
import dev.bahodir.newsappwithdagger2.adapter.browse.BrowseVpAdapter
import dev.bahodir.newsappwithdagger2.app.App
import dev.bahodir.newsappwithdagger2.databinding.CustomTabBinding
import dev.bahodir.newsappwithdagger2.databinding.FragmentBrowseBinding
import dev.bahodir.newsappwithdagger2.fragment.home.HomePageFragment
...
class BrowseFragment : Fragment(), CoroutineScope {
    ...
    private lateinit var browseVpAdapter: BrowseVpAdapter
    ...
    override fun onCreateView(
        ...
        App.appComponent.browseFragment(this)
        _binding = FragmentBrowseBinding.inflate(inflater, container, false)

        list = SelectDB.getInstance(requireContext()).selectDao().getList()

        browseVpAdapter = BrowseVpAdapter(requireActivity(), arr)
        binding.viewTransition.adapter = browseVpAdapter

        TabLayoutMediator(binding.tab, binding.viewTransition) { tab, position ->
            val bind = CustomTabBinding.inflate(layoutInflater)

            tab.customView = bind.root
            bind.texts.text = arr[position]

            if (position == 0) {
                bind.texts.setBackgroundResource(R.drawable.next_back)
                bind.texts.setTextColor(Color.WHITE)
            } else {
                bind.texts.setBackgroundResource(R.drawable.white_back)
        ...
        binding.tab.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab?) {
                val bind = CustomTabBinding.bind(tab?.customView!!)
                bind.texts.setBackgroundResource(R.drawable.next_back)
                bind.texts.setTextColor(Color.WHITE)
            }
        ...
        binding.seeAll.setOnClickListener {
            findNavController().navigate(R.id.action_homePageFragment_to_seeAllFragment)
        }
