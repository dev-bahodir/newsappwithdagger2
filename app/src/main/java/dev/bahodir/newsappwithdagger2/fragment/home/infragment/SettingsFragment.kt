...
import dev.bahodir.newsappwithdagger2.databinding.FragmentSettingsBinding
...
class SettingsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.switchButton.setOnCheckedChangeListener { compoundButton, b ->
            if (b) {
                cacheTheme.setCacheBool(true)
                ThemeHelper.applyTheme(ThemeHelper.darkMode)
            } else {
                ThemeHelper.applyTheme(ThemeHelper.lightMode)
                cacheTheme.setCacheBool(false)
            }
        }

        binding.next.setOnClickListener {
            findNavController().navigate(R.id.action_homePageFragment_to_languageFragment)
        }
